#########################################################################
# File Name: transmon_rotate.py
# Description: Defines the TransmonRotate class, which is used to draw the geometric structure of a rotated superconducting qubit and generate elements in a GDS design database.
...
class TransmonRotate(LibraryBase):
    ...
    def __init__(self, options):
        """
        Initializes the TransmonRotate class.
    ...
    def calc_general_ops(self):
        """
        Calculates and generates the coordinates for pins and outline.
        """
        ...
        self.readout_pins.append((gds_pos[0], gds_pos[1]+gap/2+height+pad_gap[1]+pad_height[1]/2+cpw_extend[1]))
        ...
        self.coupling_pins.append((-width/2-cpw_extend[0]+gds_pos[0],gap/2+height+pad_gap[0]+pad_height[0]/2+gds_pos[1]))
        self.coupling_pins.append((gds_pos[0], gds_pos[1]-gap/2-height-pad_gap[4]-pad_height[4]/2-cpw_extend[4]))
        self.coupling_pins.append((gds_pos[0]-width/2-cpw_extend[3], gds_pos[1]-gap/2-height-pad_gap[3]-pad_height[3]/2))
        ...
        # Update all coordinate points
        new_readout_pins = [rotate_point(x, y, gds_pos[0], gds_pos[1], rotate) for x, y in self.readout_pins]
        new_control_pins = [rotate_point(x, y, gds_pos[0], gds_pos[1], rotate) for x, y in self.control_pins]
        new_coupling_pins = [rotate_point(x, y, gds_pos[0], gds_pos[1], rotate) for x, y in self.coupling_pins]
        new_outline = [rotate_point(x, y, gds_pos[0], gds_pos[1], rotate) for x, y in self.outline]

        # Assign back to the original variables
        self.readout_pins = new_readout_pins
        self.control_pins = new_control_pins
        self.coupling_pins = new_coupling_pins
    ...
    def draw_gds(self):
        ...
        sub_poly.rotate(rotate, center=gds_pos)
