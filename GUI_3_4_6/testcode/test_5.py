...
class MainWindow(FramelessWindow):
    ...
    def __init__(self, parent=None):
        ...
        btn7 = QtWidgets.QPushButton('生成引脚')  # 新增按钮，放在生成读取腔后
        ...
        btn12 = QtWidgets.QPushButton('Others')  # 新增Others按钮，放在仿真后
        ...
        btn7.clicked.connect(self.MenuAffairs)  # 新增按钮的事件绑定
        ...
        btn12.clicked.connect(self.MenuAffairs)  # 绑定Others按钮事件
    ...
    def MenuAffairs(self, checked):
        ...
        if sender.objectName() == 'Topology':
            ...
            self.design.equivalent_circuit.show()
            # 暂时没用
            # self.show_circuit_image()  # 只在面板点击时显示
    ...
    def onFaceClicked(self, objectName):
        if not self.design_updated_flag:
            # print("目前还未生成图像！")
            return  # 如果设计未更新，直接返回，不执行后续操作

        if objectName == 'topoInterface':
            print("显示拓扑图像")
            self.show_topology_image(show=True)  # 只在激活此界面时显示
        elif objectName == 'CircuitInterface':
            print("显示等效电路图像")
            self.show_circuit_image(show=True)  # 只在激活此界面时显示
        elif objectName == 'GDSInterface':
            print("显示GDS图像")
            self.show_gds_image(show=True)  # 只在激活此界面时显示
    ...
    def _select_file(self, checked):
        ...
        # self.design.topology.show_image()  # 这部分可以调整，看是否需要
    ...
    def updateMainDesign(self, updated_design):
        ...
        self.design_updated_flag = True  # 设置状态标志为 True
        print("主窗口的设计已更新")
        # 可能不需要在这里调用显示图像
        self.show_topology_image(show=True)
    ...
    def show_topology_image(self, show=False):
        ...
        picture_path = './picture/topology.png'  # 图片路径
    ...
    def show_picture(self, label, picture_path):
        label.resize(500, 500)  # 确保label的大小足够显示图片
        label.setPixmap(QtGui.QPixmap(picture_path))
        print(f"显示图片: {picture_path}")  # 调试信息，确认显示的图片路径
