def add_air_bridges_czy(pos, bend_radius, spacing=120, chip_type="chip3", width=10, air_bridge_type="AirbriageNb"):
    ...
    def find_nearest_segment_center(polygons, curr_point, line_width):
        """
        Search for the center point of the line segment nearest to curr_point and adjust the center position according to the line width.
        """
        ...
        for i in range(len(path_points) - 1):
            ...
            # Calculate the distance from adjusted center point to curr_point
    ...
    def adjust_air_bridge_position_for_bend(prev_point, curr_point, next_point, bend_radius, width):
        """
        Optimize the offset of the air bridge, accurately calculate the contact points between rounded corners and paths, and dynamically adjust correction factors.
        """
        ...
        return gds_pos, rotation_angle + math.pi / 2  # Clockwise rotation by 90 degrees

    def is_point_in_flexpath(point, polygons, tolerance):
        """
        Check if the checkpoint is within the polygon range of the path, allowing for a certain tolerance.
        """
        px, py = point

        # Determine whether the point is within the polygon (strict condition)
    ...
    def do_lines_intersect(p1, p2, q1, q2):
        ...
        Parameters:
        p1, p2: Start and end points of the first line segment (x1, y1), (x2, y2)
        q1, q2: Start and end points of the second line segment (x3, y3), (x4, y4)
        
        Returns:
        bool: If intersecting, return True; otherwise return False.
        """
        ...
        def on_segment(p, q, r):
            """Check if point q is on the line segment pr"""
        ...
        # Special case
        # p1, p2 and q1 are collinear, check if q1 is on segment p1p2
        if o1 == 0 and on_segment(p1, q1, p2):
            return True
        # p1, p2 and q2 are collinear, check if q2 is on segment p1p2
        if o2 == 0 and on_segment(p1, q2, p2):
            return True
        # q1, q2 and p1 are collinear, check if p1 is on segment q1q2
        if o3 == 0 and on_segment(q1, p1, q2):
            return True
        # q1, q2 and p2 are collinear, check if p2 is on segment q1q2
    ...
    def angle_to_path_vector(angle, length):
        ...
        """
        dx = length * math.cos(angle)  # x component
        dy = length * math.sin(angle)  # y component
        return (dx, dy)

    def is_point_intersect(point, path_vector, last_point, last_path_vector):
        """
        Check if the current point's air bridge would intersect with the previous one.
        """
        def get_line(point, path_vector):
            ...
            perp_dx = -path_vector[1]  # Perpendicular direction x component
            perp_dy = path_vector[0]   # Perpendicular direction y component
        ...
        for i in range(0, 2, 1):
            for j in range(0, 2, 1):
                if do_lines_intersect(line1[i][0], line1[i][1], line2[j][0], line2[j][1]):
                    ok = 0
    ...
    def is_point_intersect_for_bend(point, path_vector, last_point, last_path_vector):
        """
        Special intersection check for bend points.
        """
        ...
        if path_length > 0:
            ...
            for j in range(1, num_bridges + 1):
                ...
                if i > 0 and j == 1:
                    ...
                    if is_point_in_flexpath(gds_pos, polygons, width / 2 + 5):  # Add 5 unit tolerance
    ...
    for i in range(1, len(pos) - 1):
        ...
        # Check if the center point meets the range conditions (this logic needs further improvement)
        if is_point_in_flexpath(adjusted_pos, polygons, width / 2 + 5):  # Add 5 unit tolerance
